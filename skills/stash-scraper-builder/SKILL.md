...
**Source policy**: Normative schema/runtime claims require a source citation; experience-based guidance is labeled `Heuristic`.
...
**Source:** Official CommunityScrapers schema — root `name` is required and unknown root keys are rejected (`additionalProperties: false`). See `references/UPSTREAM_SOURCES.md`.

### Prohibited patterns

- Do **not** emit root keys `documentHeader` or `$vars`. `name` is required; the filename conventionally supplies its value. **Source:** official schema (`additionalProperties: false`, `required: [name]`).
- URL actions: `url` is an array (case-sensitive contains-match). **Source:** official schema, `definitions.anyByURL.url`.
- Do not invent search modes (`sceneByName` / `sceneByQueryFragment`) unless the site actually has search. **Source:** project workflow; verify the real endpoint before adding the mode.
...
- **Nil pointer dereference** — This is an upstream Stash bug (v0.31.1+) that occurs when a fragment scraper returns zero rows while the scene block defines relationships (Performers/Tags/Studio). Mitigate by testing fragment modes against non-matching input before deployment. See `references/scraping-failures.md`. **Source:** Stash issue #6921; do not infer additional causality.
...
**Source:** First five structural/data rules are official-schema or Stash-development-backed; site capability, selector stability, studio semantics, and quality rules are `Heuristic` guidance and require live verification. Runtime panic guidance cites Stash issue #6921 in `scraping-failures.md`.
...
- Before editing a normative rule, verify the claim against `references/UPSTREAM_SOURCES.md` and add a nearby `Source:` or `Heuristic:` label.
